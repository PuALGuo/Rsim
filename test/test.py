...
def bit_check(x):
	...
	#condition_1 = keras.backend.less_equal(tf.floor(result*8),4)
	#condition_2 = keras.backend.greater_equal(tf.floor(result*8),4)
	...
	##global ans2
	#global ans1
	#ans2 = keras.backend.sum(keras.backend.ones_like(x), axis=(0,1,2,3)) + ans1
	#ans2 = tf.Print(ans2,[ans2])
	#result = result + ans2
	#result = result - ans2
	#ans1 = ans2
...
def summ(x):
	global ans1
	ans2 = keras.backend.sum(keras.backend.ones_like(x), axis=(0,1,2,3)) 
	ans2 = tf.Print(ans2,[ans2])
	x = x + ans2
	x = x - ans2
	return x	
...
ans1 = tf.placeholder('float32', shape = ())
#ans2 = keras.backend.variable(0)
...
#training
batch_size = 64
model.load_weights('model2.h5', by_name=True) 
opt_rms = keras.optimizers.rmsprop(lr=0.00001,decay=1e-9)
#sgd = SGD(lr=0.000001, decay=1e-9, momentum=0.9, nesterov=True)
model.compile(loss='categorical_crossentropy', optimizer=keras.optimizers.Adam(decay=0.00002), metrics=['accuracy'])
